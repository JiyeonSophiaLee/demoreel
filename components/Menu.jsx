import { memo, useMemo, useRef } from 'react';


import MenuComponent from './menu/MenuComponent.jsx';
import SvgIvory from './menu/SvgIvory.jsx'
import InfoContent from './menu/info/InfoContent.jsx'


// import { SvgFrameContext } from './HomeLayout.jsx';

function Menu(props){
  console.log('----------Menu-----------')
  return useMemo(()=>{
    return <MenuRender menuValues={props.menuValues} values={props.values}></MenuRender>
  },[])
}


function MenuRender(props){
  console.log('----------MenuRender----------')
  return (
    <section id = "menu">
      <MenuComponent values = {{id:props.menuValues[0].id,
                                order:props.menuValues[0].order,
                                stopColor:[props.menuValues[0]["stopColor"][0],props.menuValues[0]["stopColor"][1]],
                                strokeColor:[props.menuValues[0]["strokeColor"][0],props.menuValues[0]["strokeColor"][1]],
                                svgFrameValuesImmutable: props.values, 
                                ivory:<SvgIvory/> }}/>
      <MenuComponent values = {{id:props.menuValues[1].id,
                                order:props.menuValues[1].order,
                                stopColor:[props.menuValues[1]["stopColor"][0],props.menuValues[1]["stopColor"][1]],
                                strokeColor:[props.menuValues[1]["strokeColor"][0],props.menuValues[1]["strokeColor"][1]],
                                svgFrameValuesImmutable: props.values }}/>
      <MenuComponent values = {{id:props.menuValues[2].id,
                                order:props.menuValues[2].order,
                                stopColor:[props.menuValues[2]["stopColor"][0],props.menuValues[2]["stopColor"][1]],
                                strokeColor:[props.menuValues[2]["strokeColor"][0],props.menuValues[2]["strokeColor"][1]],
                                svgFrameValuesImmutable: props.values }}/>
      <MenuComponent values = {{id:props.menuValues[3].id,
                                order:props.menuValues[3].order,
                                stopColor:[props.menuValues[3]["stopColor"][0],props.menuValues[3]["stopColor"][1]],
                                strokeColor:[props.menuValues[3]["strokeColor"][0],props.menuValues[3]["strokeColor"][1]],
                                svgFrameValuesImmutable: props.values, 
                                contents:<InfoContent/>}}/>
    </section>
    )
}

export default Menu